    ...
    while True:
        ...
        for i, track_id in enumerate(tracks.keys()):
            ...
            if len(data) > 0:
                ...
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len("Gorilla")+len(str(data["individual_id"])))*17, int(bbox[1])), color, -1)
                cv2.putText(frame, "Gorilla" + " : " + str(data["individual_id"]),(int(bbox[0]), int(bbox[1]-11)),0, 1.2, (255,255,255),2, lineType=cv2.LINE_AA)    
...
if __name__ == '__main__':
    ...
    model2_path = ""
    db_folder = ""
    out_path = "/data/video33_predicted.avi"
