	...
	for dataset in datasets:
		...
		for sub_dataset in dataset['datasets']:
			...
			for id in os.listdir(os.path.join(f"{output_path}/{identifier}", "test")):
				...
				random.shuffle(files)
				for file_name in files[:round(len(files) * 0.25)]:
					if not os.path.exists(os.path.join(output_path, identifier, 'val', id)):
						os.mkdir(os.path.join(output_path, identifier, 'val', id))
