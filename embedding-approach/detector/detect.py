...
from detector.utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
...
def detect(source, weights, device_num):
    view_img = False
    save_txt = False
    trace = True
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    total_res.append([cls, conf, *xyxy])

            # Print time (inference + NMS)
            print(f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS')

    print(f'Done. ({time.time() - t0:.3f}s)')
    return dataset, total_res
